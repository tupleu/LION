...
def load_data():
    # data = torch.empty(num_input, image_size, image_size).to(device)
    data = torch.empty(num_input, num_primitives, num_features).to(device)
    for i, filename in tqdm(enumerate(sorted(os.listdir(dir_path)))):
        ...
        # data[i] = torch.swapaxes(
        #     torch.cat((positions, features, transforms), dim=1), 0, 1
        # )
        # d = torch.reshape(torch.cat((positions, features, transforms), dim=1), (image_size, image_size))
        # d = torch.transpose(torch.cat((positions, features, transforms), dim=1),0,1)
        d = torch.cat((positions, features, transforms), dim=1)
        # save(d,'test1')
        # save(preprocess(to_pil_image(d, 'F'))[0],'test2')
        # save(postprocess(to_pil_image(preprocess(to_pil_image(d, 'F')),'F'))[0],'test3')
        # exit()
        # data[i] = torch.detach(d)
        # d.unsqueeze_(2)
        # p = preprocess(d)
        # data[i] = p.reshape((num_features,num_primitives)).detach()
        data[i] = d.detach()
        # data[i] = preprocess(to_pil_image(d,'F'))
        # data[i] = torch.ones((64,64))
        if i == num_input - 1:
            break
...
class ShapeNet15kPointClouds(Dataset):
    def __init__(self,
        ...
        self.all_points = load_data().cpu().detach().numpy()
    ...
    def __getitem__(self, idx):
        ...
        cate_idx = 444#self.cate_idx_lst[idx]
        sid, mid = (555,666)#self.all_cate_mids[idx]
        input_pts = tr_out
